...
const COLUMNS: &str = "id, name, enabled, embed_image, max_tags, supports_character,
	...
	tag_spaces_as_plus, character_space_replacement,
...
impl Database {
	...
	character_space_replacement, count_url, count_path_json,
	...
	posts_url = ?14,
	posts_path_json = ?15,
	file_url_path_json = ?16,
	source_url_path_json = ?17,
	detail_url = ?18,
	detail_id_path_json = ?19,
	detail_file_url_path_json = ?20,
	detail_source_url_path_json = ?21,
	post_url = ?22,
	headers_json = ?23,
	env_params_json = ?24
	WHERE id = ?25",
...
fn read_booru_row(row: &Row<'_>) -> rusqlite::Result<BooruRow> {
	Ok(BooruRow {
		...
		count_url: row.get(12)?,
		count_path_json: row.get(13)?,
		posts_url: row.get(14)?,
		posts_path_json: row.get(15)?,
		file_url_path_json: row.get(16)?,
		source_url_path_json: row.get(17)?,
		detail_url: row.get(18)?,
		detail_id_path_json: row.get(19)?,
		detail_file_url_path_json: row.get(20)?,
		detail_source_url_path_json: row.get(21)?,
		post_url: row.get(22)?,
		headers_json: row.get(23)?,
		env_params_json: row.get(24)?,
	})
}
